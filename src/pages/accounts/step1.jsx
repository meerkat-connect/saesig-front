...
const TermsForm = ({onNext, formData, updateFormData}) => {
    ...
    const watchAllFields = watch(["agreeTOS", "agreePrivacy", "agreeGeo", "agreeMarketing"]);

    useEffect(() => {
        // const allAgreed = ["agreeTOS", "agreePrivacy", "agreeGeo", "agreeMarketing"].every(field => watchAllFields[field]);
        // const allAgreed = agreementFields.every(field => getValues(field));
        const allAgreed = watchAllFields.every(value => value);
        setAllChecked(allAgreed)
    }, [watchAllFields])

    const handleAllCheck = (e) => {
        const checked = e.target.checked;
        setValue("agreeTOS", checked);
        setValue("agreePrivacy", checked);
        setValue("agreeGeo", checked);
        setValue("agreeMarketing", checked);
        setAllChecked(checked);
    }

    const onSubmit = (data) => {
        if (!data.agreeTOS || !data.agreePrivacy) {
            setShowError(true);
            return;
        }
        updateFormData(data);
    ...
    return (
        <div id="signup-1" className="signup">
            ...
            {/* tooltip */}
            {showError && <div className="ss-notification">필수 항목을 동의해야 회원가입이 가능합니다</div>}
                ...
                <div className="ss-form">
                    <ul className="ss-form__col-container mb-60">
                        ...
                        <li className="between">
                            ...
                            <button className="ss-text-button --underline" type="button" onClick={handleViewButtonClick}>보기</button>
                        </li>
                        <li className="between">
                            <input
                                className="ss-input"
